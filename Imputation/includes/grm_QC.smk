...
rule grm:
    ...
    resources:
        mem_per_thread_gb=lambda wildcards, attempt: attempt * grm_QC_dict["grm_QC"],
        disk_per_thread_gb=lambda wildcards, attempt: attempt * grm_QC_dict["grm_QC"]
    ...
    shell:
        """
        singularity exec {params.sif} awk '$1 == "2324"' {input.bim} | singularity exec {params.sif} cut -f2 > {params.out}/y_par.snps ### Y SNPs are coded as XY => after changing to numbers = 2324
        singularity exec {params.sif} plink --bfile {params.het_filter} --exclude {params.out}/y_par.snps --make-bed --out {params.out}/grm --noweb
        singularity exec {params.sif} gcta64 --bfile {params.out}/grm --make-grm --out {params.out}/grm
        """
...
#     shell:
#         """
#         singularity exec {params.sif} gcta64 --grm {params.grm} --grm-singleton 0.05 --out {params.grm_singleton}
#         """
...
rule grm_subset:
    ...
    shell:
        """
        singularity exec {params.sif} plink --bfile {params.het_filter} --keep {input.grm_filter_id} --make-bed --out  {params.grm_subset}
        """
