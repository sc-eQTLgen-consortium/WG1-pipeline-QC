...
rule separate_ethnicities:
    ...
    resources:        
        mem_per_thread_gb=lambda wildcards, attempt: attempt * GTM_check_dict["separate_ethnicities_memory"],
        disk_per_thread_gb=lambda wildcards, attempt: attempt * GTM_check_dict["separate_ethnicities_memory"]
    threads: GTM_check_dict["separate_ethnicities_threads"]
    params:
            sif = input_dict["singularity_image"],
    shell:
        """
        ### Pull the individuals that are Finnish_in_Finland or Northern_and_Western_European_Ancestry >> European file
        grep "Finnish_in_Finland" {input} | awk ... > {output.european}
        grep "Northern_and_Western_European_Ancestry" {input} | awk ... >> {output.european}

        ### All other individuals >> admixed file
        grep "Finnish_in_Finland" {input} | awk ... > {output.non_european}
        grep "Northern_and_Western_European_Ancestry" {input} | awk ... >> {output.non_european}
        """
