...
##### Read in Results #####
sex_check <- fread(args$sex_check, sep = "\t")
colnames(sex_check)[1] <- "#FID"
sex_decisions <- fread(args$sex_decisions, sep = "\t")

psam <- fread(args$psam)
ancestry_decisions <- fread(args$ancestry_decisions)
...
if(nrow(sex_decisions) > 0){
    ...
    ggsave(plot_grid(p_sex1, p_sex2, rel_heights = c(2.5, 2), ncol = 1, nrow = 2, align = "v", axis = "lr"), filename = paste0(args$out, "/sex_summary.png"), width = 4)


} else {
    ggsave(p_sex1, filename = paste0(args$out, "/sex_summary.png"))
}
...
if(nrow(ancestry_decisions) > 0){
    ...
    ggsave(plot_grid(p_ancestry1, p_ancestry2, rel_heights = c(2.5, 2), ncol = 1, nrow = 2, align = "v", axis = "lr"), filename = paste0(args$out, "/ancestry_summary.png"), width = 2 + length(unique(ancestry_combined_long$Ancestry)))


} else {
    ggsave(p_ancestry1, filename = paste0(args$out, "/sex_summary.png"))
}
