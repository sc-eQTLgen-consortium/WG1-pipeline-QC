...
if (!is.null(args$demuxlet)) {
	...
	results_list[["demuxlet"]]$Demuxlet_DropletType <- gsub("SNG", "singlet", results_list[["demuxlet"]]$Demuxlet_DropletType) %>%
		gsub("DBL", "doublet", .) %>%
		gsub("AMB", "unassigned", .)
...
if (!is.null(args$freemuxlet)) {
	...
	### update Demuxlet_DropletType  column SNG and DBL ###
	results_list[["Freemuxlet"]]$Freemuxlet_DropletType <- gsub("SNG", "singlet", results_list[["Freemuxlet"]]$Freemuxlet_DropletType) %>%
		gsub("DBL", "doublet", .) %>%
		gsub("AMB", "unassigned", .)
...
if (!is.null(args$freemuxlet_assignments) |
	!is.null(args$scSplit_assignments) |
	!is.null(args$souporcell_assignments)) {
	### Make a list to store the results that will be combined in ###
...
} else if (is.null(args$demuxlet) &
	is.null(args$vireo) &
	length(cluster_no_assign) == 1) {
...
message("\nWriting output.\n")
data.table::fwrite(combined_results, paste0(args$out, "combined_results.tsv.gz"), sep = "\t", append = FALSE)
...
if (!is.null(args$demuxlet) |
	!is.null(args$freemuxlet) |
	!is.null(args$scSplit) |
	!is.null(args$souporcell) |
	!is.null(args$vireo)) {
...
data.table::fwrite(combined_results_summary, paste0(args$out, "combined_results_summary.tsv.gz"), sep = "\t", append = FALSE)
...
if (is.null(args$method)) {
	...
	data.table::fwrite(combined_results, paste0(args$out, "combined_results_w_combined_assignments.tsv.gz"), sep = "\t", append = FALSE)
	...
	data.table::fwrite(summary_combined_results, paste0(args$out, "Final_Assignments_demultiplexing_doublets.tsv.gz"), sep = "\t", append = FALSE)
