...
rule souporcell:
    input:
        ...
        common_variants = input_dict["common_variants"]
    resources:
        mem_per_thread_gb = lambda attempt: attempt * souporcell_dict["souporcell_memory"],
        disk_per_thread_gb = lambda attempt: attempt * souporcell_dict["souporcell_memory"]
    ...
    params:
        ...
        clusters = lambda wildcards: souporcell_dict[wildcards.pool],
...
rule souporcell_summary:
    ...
    resources:
        mem_per_thread_gb = lambda attempt: attempt * souporcell_dict["souporcell_summary_memory"],
        disk_per_thread_gb = lambda attempt: attempt * souporcell_dict["souporcell_summary_memory"]
...
rule souporcell_pool_vcf:
    input:
        ...
        cluster_genotypes = output_dict["output_dir"] + "/{pool}/souporcell/cluster_genotypes.vcf"
    output:
    ...
    resources:
    resources:
        mem_per_thread_gb = lambda attempt: attempt * souporcell_dict["souporcell_pool_vcf_memory"],
        disk_per_thread_gb = lambda attempt: attempt * souporcell_dict["souporcell_pool_vcf_memory"]
    ...
    shell:
        """
        singularity exec --bind {params.bind} {params.sif} bedtools intersect -a {input.genotypes} -b {input.cluster_genotypes} -f 1.0 -r -wa -header > {output.filtered_refs_temp} 2> {log}
